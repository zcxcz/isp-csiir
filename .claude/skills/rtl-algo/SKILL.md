...
RTL 开发周期相对较长，试错成本较高。当初版算法确认后，后续如有算法变更需求，rtl-algo 必须遵循规范化流程处理。

### 变更请求处理步骤

| 步骤 | 输入 | 输出 | 说明 |
|------|------|------|------|
| 1. 可行性评估 | 变更需求描述 | 可行性评估报告 | 评估硬件实现可行性 |
| 2. 浮点模型更新 | 可行性确认 | 更新的浮点模型 | 保持算法精度 |
| 3. 定点模型更新 | 浮点模型 | 更新的定点模型 | 量化定点化影响 |
| 4. 提交确认 | 完整模型 | 确认请求 | 提交 rtl-pm 请求用户确认 |
...
```
变更请求 → rtl-algo 可行性评估 → [不可行] → 拒绝/重新设计
                ↓ [可行]
         更新浮点模型
                ↓
         更新定点模型
