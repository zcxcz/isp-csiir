...
//   - Weighted sum calculation for 5 directions
...
module stage2_directional_avg #(
    ...
    // 5x5 Window input (delayed from Stage 1)
    ...
    reg [DATA_WIDTH-1:0] win_s4 [0:4][0:4];
    reg                  valid_s4;
    ...
    // Cycle 5: Weighted Sum (First Stage)
    //=========================================================================
    // Define weight kernels for different sizes
    // For simplicity, use uniform weights for each direction
    // Center: full weight, edges: partial weight

    // Weight assignments based on kernel selection
    // Using simple averaging: each direction gets 1/N weight

    // Calculate sums for each direction (5x5 window)
    // Center (C): sum of all 25 pixels
    wire [ACC_WIDTH-1:0] sum_center_comb = win_s4[0][0] + win_s4[0][1] + win_s4[0][2] + win_s4[0][3] + win_s4[0][4] +
    ...
    wire [ACC_WIDTH-1:0] sum_up_comb = win_s4[0][0] + win_s4[0][1] + win_s4[0][2] + win_s4[0][3] + win_s4[0][4] +
                                       win_s4[1][0] + win_s4[1][1] + win_s4[1][2] + win_s4[1][3] + win_s4[1][4] +
                                       win_s4[2][0] + win_s4[2][1] + win_s4[2][2] + win_s4[2][3] + win_s4[2][4];

    // Down (D): sum of bottom 3 rows
    wire [ACC_WIDTH-1:0] sum_down_comb = win_s4[2][0] + win_s4[2][1] + win_s4[2][2] + win_s4[2][3] + win_s4[2][4] +
                                         win_s4[3][0] + win_s4[3][1] + win_s4[3][2] + win_s4[3][3] + win_s4[3][4] +
                                         win_s4[4][0] + win_s4[4][1] + win_s4[4][2] + win_s4[4][3] + win_s4[4][4];

    // Left (L): sum of left 3 columns
    wire [ACC_WIDTH-1:0] sum_left_comb = win_s4[0][0] + win_s4[0][1] + win_s4[0][2] +
    ...
    wire [ACC_WIDTH-1:0] sum_right_comb = win_s4[0][2] + win_s4[0][3] + win_s4[0][4] +
    ...
    reg [ACC_WIDTH-1:0] sum_center_s5, sum_up_s5, sum_down_s5, sum_left_s5, sum_right_s5;
    reg                 valid_s5;
    reg [2:0]           kernel_s5;
    ...
    reg [ACC_WIDTH-1:0] sum_c_s6, sum_u_s6, sum_d_s6, sum_l_s6, sum_r_s6;
    ...
    wire [DATA_WIDTH-1:0] avg0_c_comb = (w_c_s6 != 0) ? (sum_c_s6 / w_c_s6) : {DATA_WIDTH{1'b0}};
    wire [DATA_WIDTH-1:0] avg0_u_comb = (w_u_s6 != 0) ? (sum_u_s6 / w_u_s6) : {DATA_WIDTH{1'b0}};
    wire [DATA_WIDTH-1:0] avg0_d_comb = (w_d_s6 != 0) ? (sum_d_s6 / w_d_s6) : {DATA_WIDTH{1'b0}};
    wire [DATA_WIDTH-1:0] avg0_l_comb = (w_l_s6 != 0) ? (sum_l_s6 / w_l_s6) : {DATA_WIDTH{1'b0}};
    wire [DATA_WIDTH-1:0] avg0_r_comb = (w_r_s6 != 0) ? (sum_r_s6 / w_r_s6) : {DATA_WIDTH{1'b0}};

    // avg1 uses same calculation (for this simplified implementation)
    wire [DATA_WIDTH-1:0] avg1_c_comb = avg0_c_comb;
    wire [DATA_WIDTH-1:0] avg1_u_comb = avg0_u_comb;
    wire [DATA_WIDTH-1:0] avg1_d_comb = avg0_d_comb;
    wire [DATA_WIDTH-1:0] avg1_l_comb = avg0_l_comb;
    wire [DATA_WIDTH-1:0] avg1_r_comb = avg0_r_comb;
