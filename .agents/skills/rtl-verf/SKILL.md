...
所有验证脚本、compare 脚本、trace dump 脚本在启动 RTL 仿真时，必须默认实现以下机制：

- 必须同时具备 TB 内 watchdog 和脚本外层 wall-clock timeout；禁止只依赖其中一层。
- 外层 timeout 触发后，必须清理**整个仿真进程组**，不能只结束父 Python 进程或 shell 包装进程。
- 禁止留下后台 `vvp`、`iverilog`、trace dump 可执行文件或其他孤儿仿真进程；验证结束后应默认做到“无残留”。
- 对已知可能卡死的 directed case / trace replay / debug compare，默认优先设置较小 timeout，先拿超时结论，再决定是否扩大样本。
- 若发生超时，脚本输出中必须明确写清：
  - timeout 类型（TB watchdog / outer timeout）
  - case 名称、尺寸、seed
  - 已执行的清理动作（例如已 kill process group）
- 若发现一次验证留下后台仿真进程，本轮验证流程结论应降级，不得宣称“流程已收敛”。

执行约束：
- 新增验证入口时，仿真启动逻辑必须复用统一 helper，或实现等价的“新进程组 + timeout + 清组”机制；禁止裸 `subprocess.run([sim])` 无保护运行长仿真。
- 修改旧脚本时，若发现仍存在裸跑仿真入口，应顺手补齐上述机制，不要留技术债继续扩散。
- 验证结束前，默认应检查是否存在与本轮 case 对应的残留仿真进程；若有，先清理，再汇报结果。

## 黄金数据来源铁律

本 repo 的算法语义来源默认且唯一为**定点化模型**。验证过程中必须遵守以下硬约束：

- 所有 `expect` / `golden` / `reference` 数据，必须由定点化模型直接导出，或由调用定点化模型的导出脚本生成。
- directed case 也不例外；如果是 directed semantic case，expected 仍必须先由定点化模型导出成向量、trace 或文件，再由 TB 读取比对。
- 严禁在 SV testbench、辅助 Python、脚本或文档示例中私自手写算法 expected。
- 严禁在 TB 内重写一套“影子参考”，包括但不限于：
  - 手写 LUT / clip / threshold bucket
  - 手写 kernel 选择
  - 手写窗口边界复制语义
  - 手写平均值 / 插值 / rounding
  - 手写 patch / 坐标 / metadata 的算法期望
- TB 内允许保留的本地逻辑，仅限握手对齐、延迟对齐、文件读写、向量搬运和非算法性的 protocol checker。

执行约束：
- 若当前定点化模型还不能直接产出某子模块的 expected，先补模型导出能力，再写/改 TB；不要在 TB 里临时手推 expected 顶上。
- 每个关键 TB 顶部必须明确写清：`expected_source`、`golden_artifact`、`golden_generator`、`compare_scope`。
- 若发现 expected 不是定点化模型导出，而是本地私有生成，本轮验证结论直接降级为 `tb_gap_identified`，不得据此宣称“子模块已收敛”。
...
- 对 linebuffer、window generator、patch assembler 一类边界敏感模块，必须在设计和验证阶段显式冻结 padding 语义。
- padding 语义至少要明确：top / bottom / left / right / corner 的复制、镜像、补零或其他策略；未明确前，不得默认“按常识处理”。
- 若 ref 不直接建模 linebuffer 行为，也必须把 padding 语义写成独立 contract，并针对该 contract 做 directed + random 专项验证，保证子模块功能完整。
...
- 设计 directed case 时，先定义“要让定点化模型导出什么对象”；若该对象不在模型范围内，例如 linebuffer padding/window contract，则先把 contract 写清，再设计对该 contract 的专项 case。
- 对 linebuffer 这类模块，默认至少覆盖：top row、bottom row、left edge、right edge、corner、center 区域，以及 max-width / flush / backpressure 下的窗口稳定性。
