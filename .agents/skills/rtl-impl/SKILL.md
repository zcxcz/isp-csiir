...
算法模块 IP 开发默认优先参考仓库模板 `rtl/ip_template.v`。该文件定义了推荐的接口分区、组合逻辑在前、`common_pipe_slice` 在后、`PIPE_TYPE=0` 作为默认 pipeline 级间寄存器的写法，同时统一采用 `pipe_sX_din` / `pipe_sX_dout` / `valid_sX` 的级间命名风格。多级 pipeline 时按 `u_pipe_s0 -> u_pipe_s1 -> ...` 串联，并使用后级 `ready_out` 驱动前级 `ready_in`，便于维护和扩展。
...
// 第一级：组合逻辑
//====================================================================
assign stage1_result = din + 1'b1;

//====================================================================
// 第二级：流水线寄存器
//====================================================================
// 算法模块 pipeline 一般默认采用 common_pipe_slice 的 PIPE_TYPE=0。
// 这种写法输出完全寄存器化，更适合多级流水和时序收敛。
common_pipe_slice #(
...
// 算法模块的流水线级间寄存器，默认使用 PIPE_TYPE=0
common_pipe_slice #(
    .DATA_WIDTH (WIDTH),
    .RESET_VAL  (0),
    .PIPE_TYPE  (0)
...
- 时序分析清晰
- 综合工具优化效果好
- 输出完全由寄存器驱动，适合作为算法模块 pipeline 默认写法
- 避免每一级都使用 skid buffer 导致 ready 链和旁路 mux 过长
...
**实践建议：**
- 算法模块 pipeline 默认使用 `common_pipe_slice` 且 `PIPE_TYPE=0`
- `PIPE_TYPE=1` 仅在确实需要弹性缓冲、局部解耦或吞吐兜底时使用
- 若后级连接 CDC 模块、async 模块或要求输入必须来自寄存器的边界单元，应使用 `PIPE_TYPE=0`
...
module common_pipe_slice #(
    ...
    // PIPE_TYPE=0:
    //   Registered slice, recommended default for algorithm pipeline stages.
    //   Output is fully registered and timing closure is easier.
    //
    // PIPE_TYPE=1:
    //   Skid buffer, used when elastic buffering is required.
    //   Do not use as the default choice for every pipeline stage.
