#!/usr/bin/env python3
"""
Compare top-level stage3 pre-sort raw gradient trace against fixed-model local
evaluation using the traced stage1 input patches as the compare object.
"""
...
def parse_stage3_raw_trace(path: Path):
    entries = []
    current = None
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare top stage3 raw trace against fixed-model local gradients from traced stage1 patches"
    )
    parser.add_argument("--output", "-o", default="verification_results_stage3_raw_trace")
    ...
    test_dir = output_dir / f"stage3_raw_trace_{args.pattern}_{args.width}x{args.height}_seed{args.seed}"
    ...
    actual = parse_stage3_raw_trace(test_dir / "stage3_raw_trace.txt")
    ...
    for idx, act in enumerate(actual):
        ...
        for key in ("c", "u", "d", "l", "r"):
            if int(exp["g_raw"][key]) != int(act["g_raw"][key]):
                first_issue = ("g_raw", idx, key, exp, act)
                break
        if first_issue is not None:
            break
    print("=" * 60)
    print("Stage3 Raw Trace Compare")
    print("=" * 60)
    print(f"Expected trace entries: {len(actual)}")
    print(f"Actual trace entries:   {len(actual)}")
    if first_issue is None:
        print("PASS: stage3 raw trace matches fixed-model local gradients from traced stage1 patches")
        return 0
