...
module tb_debug_pipe2;
    ...
    always @(posedge clk) begin
        if (rst_n && dut.s1_valid && s1_cnt < 3) begin
            s1_cnt++;
            $display("[%0t] S1: px=%0d py=%0d", $time, dut.s1_pixel_x, dut.s1_pixel_y);
        end
        if (rst_n && dut.s2_valid && s2_cnt < 3) begin
            s2_cnt++;
            $display("[%0t] S2: px=%0d py=%0d avg0_c=%0d", $time, dut.s2_pixel_x, dut.s2_pixel_y, $signed(dut.s2_avg0_c));
        end
        if (rst_n && dut.s3_valid && s3_cnt < 3) begin
            s3_cnt++;
            $display("[%0t] S3: px=%0d py=%0d", $time, dut.s3_pixel_x, dut.s3_pixel_y);
        end
        if (rst_n && dut.s4_dout_valid && s4_cnt < 3) begin
            s4_cnt++;
    ...
    initial begin
        ...
        $display("=== Sending frame with VSYNC ===");
        send_frame();
