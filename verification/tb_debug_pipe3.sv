...
module tb_debug_pipe3;
    ...
    always @(posedge clk) begin
        if (rst_n && dut.s1_valid) s1_cnt <= s1_cnt + 1;
        if (rst_n && dut.s2_valid) s2_cnt <= s2_cnt + 1;
        if (rst_n && dut.s3_valid) s3_cnt <= s3_cnt + 1;
        if (rst_n && dut.s4_dout_valid) s4_cnt <= s4_cnt + 1;
