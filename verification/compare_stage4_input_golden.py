#!/usr/bin/env python3
"""
Compare top-level stage4 input trace against the realtime-model integrated stage4
input trace.
"""
...
from isp_csiir_realtime_model import ISPCSIIRRealtimeModel
from run_golden_verification import (
    build_default_config,
    derive_model_timeout_seconds,
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare top stage4 input trace against realtime-model integrated stage4 input trace"
    )
    parser.add_argument("--output", "-o", default="verification_results_stage4_input_golden")
    ...
    test_dir = output_dir / f"stage4_input_golden_{args.pattern}_{args.width}x{args.height}_seed{args.seed}"
    ...
    model_timeout_sec = derive_model_timeout_seconds(args.width, args.height)
    model = ISPCSIIRRealtimeModel(config, timeout_sec=model_timeout_sec)
    expected = model.export_stage4_input_trace(
        image,
    ...
    print("=" * 60)
    print("Stage4 Input Golden Compare")
    print("=" * 60)
    print(f"Expected trace entries: {len(expected)}")
    print(f"Actual trace entries:   {len(actual)}")
    if first_issue is None:
        print("PASS: stage4 input trace matches realtime-model integrated trace")
        return 0
